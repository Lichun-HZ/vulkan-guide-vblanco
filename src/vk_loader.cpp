...
std::optional<std::shared_ptr<LoadedGLTF>> loadGltf(VulkanEngine* engine,std::string_view filePath)
{
    ...
    // create buffer to hold the material data. 所有材质的参数，都放到一个buffer中，记录各自的偏移值即可。
    ...
    for (fastgltf::Material& mat : gltf.materials) {
        ...
        materialResources.dataBufferOffset = data_index * sizeof(GLTFMetallic_Roughness::MaterialConstants); // 记录buffer的offset
    ...
    for (fastgltf::Node& node : gltf.nodes) {
        ...
        std::visit(fastgltf::visitor { [&](fastgltf::Node::TransformMatrix matrix) {
                       ...
                       [&](fastgltf::Node::TRS transform) {
                           ...
                           newNode->localTransform = tm * rm * sm; // 变换顺序，s, r, t
                       } },
            node.transform);
    }
