...
AllocatedBuffer VulkanEngine::create_buffer(size_t allocSize, VkBufferUsageFlags usage, VmaMemoryUsage memoryUsage)
{
    ...
    vmaallocInfo.flags = VMA_ALLOCATION_CREATE_MAPPED_BIT;  // 分配后自动map，指针保存到newBuffer.info.pMappedData中
...
GPUMeshBuffers VulkanEngine::uploadMesh(std::span<uint32_t> indices, std::span<Vertex> vertices)
{
    ...
    //create vertex buffer，虽然是作为vertex buffer，但这里创建的是SSBO，而不是VK_BUFFER_USAGE_VERTEX_BUFFER_BIT
    newSurface.vertexBuffer = create_buffer(vertexBufferSize, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
        VMA_MEMORY_USAGE_GPU_ONLY);

    //find the adress of the vertex buffer, 需要buffer有VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT标志
...
{
    // create scene data DescriptorLayout
    {
    ...
    // Compute Shader输出的imageView
    DescriptorWriter writer;
    writer.write_image(0, _drawImage.imageView, VK_NULL_HANDLE, VK_IMAGE_LAYOUT_GENERAL, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE);

    writer.update_set(_device,_drawImageDescriptors); // 需要写的DescriptorSet
...
void GLTFMetallic_Roughness::build_pipelines(VulkanEngine* engine)
{
    ...
    // Push Constant中包含worldMatrix和vertex buffer的device address
    // DescriptorSet 0: SceneData
    //          bind 0: uniform buffer - GPUSceneData
    // DescriptorSet 1: material params
    //          bind 0: uniform buffer - MaterialConstants
    //          bind 1: combined image sampler - colorImage and sampler
    //          bind 2: combined image sampler - metalRoughImage and sampler
...
void VulkanEngine::init_mesh_pipeline()
{
    ...
    VkPipelineLayoutCreateInfo pipeline_layout_info = vkinit::pipeline_layout_create_info();
    pipeline_layout_info.pPushConstantRanges = &bufferRange;          // Push Constant
    pipeline_layout_info.pushConstantRangeCount = 1;
    pipeline_layout_info.pSetLayouts = &_singleImageDescriptorLayout; // Descriptor Layouts
...
void VulkanEngine::draw_geometry(VkCommandBuffer cmd)
{
    ...
    for (uint32_t i = 0; i < mainDrawContext.OpaqueSurfaces.size(); i++) {
        if (is_visible(mainDrawContext.OpaqueSurfaces[i], _sceneData.viewproj)) {
